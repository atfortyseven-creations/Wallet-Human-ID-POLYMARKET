...
if [ ! -f "$SDK_DIR/node_modules/@aztec/aztec.js/package.json" ]; then
  echo "ERROR: SDK 5.x no instalado. Ejecuta install-sdk5.sh primero."
  exit 1
fi
...
async function main() {
  ...
  const accountManager = new AccountManager(node, secretKey, accountContract);
  ...
  const deployTx = TokenContract.deploy(wallet, addr, 'Quantum Dots', 'QDs', 18n)
    .send({ universalDeploy: true });
  
  process.stdout.write('\n         esperando minado en testnet...');
  const receipt  = await deployTx.wait({ timeout: 600 });
  console.log('');
