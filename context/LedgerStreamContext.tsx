 ...
 * LedgerStreamContext
 *
 * Provides real-time ledger alert events from /api/ledger-stream (SSE)
 * to any component in the React tree.
 *
 * Usage:
 *   const { events, isConnected, connectionError } = useLedgerStream();
 *
 * Features:
...
const LedgerStreamContext = createContext<LedgerStreamState>({
    events: [],
    isConnected: false,
...
export function LedgerStreamProvider({ children }: { children: React.ReactNode }) {
    ...
    const connect = useCallback(() => {
        ...
        es.addEventListener('ledger', (e: MessageEvent) => {
            if (!isMounted.current) return;
            try {
                const raw = JSON.parse(e.data) as Omit<LedgerEvent, 'id'>;
    ...
    return (
        <LedgerStreamContext.Provider value={{ events, isConnected, connectionError, clearEvents }}>
            {children}
...
export function useLedgerStream(): LedgerStreamState {
