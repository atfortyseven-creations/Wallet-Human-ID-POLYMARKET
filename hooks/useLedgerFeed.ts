...
function deriveSignals(items: any[]): LedgerSignal[] {
    ...
    if (accumulation.length >= 3) {
        signals.push({
            ...
            body: `Ledgers are aggressively accumulating ${[...new Set(accumulation.map(t => t.asset))].join(', ')}. Strong bullish conviction.`,
            btcAmount: 0,
...
export function useLedgerFeed() {
    ...
    const prefsQuery = useQuery({
        queryKey: ['ledger', 'preferences'],
        queryFn: async () => {
            const res = await fetch('/api/network/ledger/preferences');
    ...
    const watchQuery = useQuery({
        queryKey: ['ledger', 'watchlist'],
        queryFn: async () => {
            const res = await fetch('/api/network/ledger/watch');
    ...
    const activityQuery = useQuery<LedgerActivity[]>({
        queryKey: ['network', 'ledger', 'activity'],
        queryFn: async () => {
            const res = await fetch('/api/ledger-events?limit=70');
            if (!res.ok) throw new Error('Failed to fetch indexed ledger events');
            const data = await res.json();
            // Map Data Lake Events to expected LedgerActivity interface exactly
    ...
    const updatePrefsMutation = useMutation({
        mutationFn: async (vars: { threshold?: number; muted?: boolean }) => {
            const res = await fetch('/api/network/ledger/preferences', { method: 'POST', body: JSON.stringify(vars) });
            return res.json();
        },
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['ledger', 'preferences'] })
    });

    const toggleWatchMutation = useMutation({
        mutationFn: async (vars: { address: string; toggle: boolean }) => {
            const res = await fetch('/api/network/ledger/watch', { method: 'POST', body: JSON.stringify(vars) });
            return res.json();
        },
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['ledger', 'watchlist'] })
    });
    ...
    const unifiedLedgerFeed = useMemo(() => {
    ...
    const mergeLedgerEvents = useVIPStore((state: VIPStoreState) => state.mergeLedgerEvents);
    const lastHashRef = useRef('');

    useEffect(() => {
        if (!unifiedLedgerFeed.length) return;

        const currentHash = `${unifiedLedgerFeed.length}-${unifiedLedgerFeed[0]?.id}`;
        if (lastHashRef.current === currentHash) return;
        lastHashRef.current = currentHash;

        const events = unifiedLedgerFeed.map(item => ({
        ...
        mergeLedgerEvents(events as any);
    }, [unifiedLedgerFeed, mergeLedgerEvents]);

    const signals = useMemo(() => deriveSignals(unifiedLedgerFeed), [unifiedLedgerFeed]);
    const flow = useMemo(() => deriveFlow(unifiedLedgerFeed), [unifiedLedgerFeed]);
    const leaderboard = useMemo(() => deriveLeaderboard(unifiedLedgerFeed), [unifiedLedgerFeed]);
    const ledgers = useMemo(() => unifiedLedgerFeed.filter(t => t.chain === 'BITCOIN' && t.amount > 500), [unifiedLedgerFeed]);
    const bscLedgers = useMemo(() => activities.filter(a => a.chain === 'BSC'), [activities]);
    const baseLedgers = useMemo(() => activities.filter(a => a.chain === 'BASE'), [activities]);
    ...
    return {
        txs, ledgers, activities, bscLedgers, baseLedgers, signals, flow, newAlerts, syncProgress, watchedAddresses, isMuted, alertThresholdBtc,
