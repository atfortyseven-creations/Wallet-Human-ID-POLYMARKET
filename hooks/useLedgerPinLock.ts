...
const PIN_VAULT_KEY_PREFIX = 'ledger_pin_hash_';
const LOCK_TIMEOUT_KEY = 'ledger_lock_timeout';
...
async function hashPIN(pin: string): Promise<string> {
  ...
  const hashBuffer = await crypto.subtle.deriveBits(
    {
      name: 'PBKDF2',
      salt: encoder.encode('ledger-pin-secure-salt-v2'),
      iterations: 250_000,
      hash: 'SHA-256',
    },
    keyMaterial,
...
export function useLedgerPinLock(address: string) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (stored) {
          const lastUnlock = parseInt(localStorage.getItem(`ledger_last_unlock_${address}`) ?? '0', 10);
  ...
  const verifyPIN = useCallback(async (pin: string): Promise<boolean> => {
    ...
    try {
      ...
      if (hash === stored) {
        setIsLocked(false);
        localStorage.setItem(`ledger_last_unlock_${address}`, String(Date.now()));
