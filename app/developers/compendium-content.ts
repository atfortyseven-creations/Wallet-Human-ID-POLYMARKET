...
export const INTRODUCTION_PARAGRAPHS: Paragraph[] = [
  ...
  { type: "p", text: "The following documentation specifies, in exhaustive technical detail, how each of these axioms is operationalized in the Whale Network system." },
  { type: "flowchart" }
];
    ...
    sections: [
      {
        ...
        content: [
          ...
          { type: "p", text: "The Whale Network architecture does not evaluate inbound volume statically; rather, it computes I(T) in real time, isolating transactional noise from the mempool to project the directional price vector prior to block finality. This temporal differential  between the intention of the transaction and its irreversible execution  constitutes the system's most valuable analytics window." },
          ...
          { type: "p", text: "Whale Network iteratively reconstructs this state by parsing Mint, Burn, and Swap logs emitted by the EVM. If a whale's order traverses N ticks, the slippage is modeled by crossing the active liquidity frontier for each tick:" },
      ...
      },
      {
        ...
        content: [
          ...
          { type: "p", text: "Consider a network C_k within a multichain topology. The net exchange in a temporal cluster t is quantified as the vector formed by the sum of inbound and outbound flows across all bridges connected to that network. Whale Network indexes Deposit and Withdrawal events across canonical bridge contracts. The early detection of a scalar asymmetry exceeding the empirical anomaly threshold indicates strategic repositioning, preempting the injection of liquidity into the target L2." },
          ...
          { type: "p", text: "By capturing the initiation transaction hash on the origin network, Whale Network permits the VIP subscriber to align positions on the destination network well before the oracle finalizes the state. This anticipation represents a temporal analytics advantage absent in standard on-chain data paradigms." },
          { type: "h4", text: "1.2.3 Algorithmic Detection of Covert Accumulation (TWAP/VWAP)" },
          { type: "p", text: "Heavy actors deploy TWAP (Time-Weighted Average Price) or VWAP (Volume-Weighted Average Price) contracts to conceal directional motives. Naive tracking overlooks transfers falling beneath the minimal alert threshold. The mathematical engine of Whale Network unifies these micro-flows by tracking the router's state variable via an exponential decay model:" },
      ...
      },
      {
        ...
        content: [
          { type: "p", text: "The existence of Whale Network introduces the Soros reflexive effect to the protocol: the act of observing the blockchain alters the behavior of the observed entities. A whale cognizant that its maneuvers spawn real-time alerts capable of moving the market adversely will adopt progressively sophisticated obfuscation strategies. The system must anticipate this adaptive behavior." },
    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          ...
          { type: "p", text: "Ethereum's JSON-RPC protocol is grossly inefficient for high-frequency paradigms: JSON payloads are plaintext, carrying an exorbitant serialization overhead. For the sovereign conduit, Whale Network transcodes the stream into Google's Protocol Buffers format, yielding an average 3-5x payload contraction and 5-10x serialization acceleration juxtaposed against identical JSON datasets." },
          { type: "code", text: `// Canonical Whale Network Protobuf Schema v4
syntax = "proto3";
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          { type: "p", text: "Regulatory capture mathematically occurs precisely when bureaucratic architecture preferentially solidifies established incumbents  colossal financial institutions wielding unfettered access to system private data  systematically handicapping disruptive challengers: open source transparency monoliths like Whale Network. Architecting preemptive defenses against this exact vector is not born of sovereign paranoia; it embodies the ruthless orchestration of systems biologically programmed to outlive hostile regulatory environments." },
    ...
    sections: [
      {
        ...
        content: [
          ...
          { type: "p", text: "The stateRoot vector operates as the cryptographic anchor of the overarching Merkle Patricia tree embodying total systemic account realities. Whale Network rigorously audits the stateRoot of every incoming structural block: any fractional mathematical discrepancy segregating the local node instance from the beacon chain's published hash emphatically signals an escalating reorg condition or node mutation, instantly defaulting into automated architectural reconciliation loops." },
