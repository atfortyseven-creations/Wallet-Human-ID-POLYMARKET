...
export const DISCLAIMER = `This technical white paper has not been approved by any regulatory authority. It is published strictly for informational and developer-documentation purposes. The information contained herein represents the current state of the Humanity Ledger Protocol architecture and is subject to change as research and development progresses. All mathematical formulations presented are operational axioms underpinning the system's inference engine, not investment advice.`;

export const INTRODUCTION_PARAGRAPHS: Paragraph[] = [
  { type: "p", text: "Humanity Ledger was founded with a single deterministic mandate: to eliminate the epistemic asymmetry between sovereign and retail participants in decentralised financial markets. Unlike traditional financial systems where privileged information flows through private terminals, blockchain networks publish every state transition  every transfer, every swap, every liquidation  to a global, immutable, publicly-auditable ledger in real time." },
  { type: "p", text: "This radical transparency is simultaneously the most powerful feature and the most underexploited primitive of public blockchain infrastructure. The raw data is available to everyone; the structured analytics derived from that data is available to almost no one. Humanity Ledger exists to close this gap." },
  ...
  { type: "p", text: "Some of the core axioms underlying the Humanity Ledger Protocol are:" },
  { type: "list", items: [
    "Information asymmetry in public blockchain networks is a quantifiable, systematic, and correctable engineering problem  not an inherent property of decentralised systems.",
  ...
  { type: "p", text: "The following documentation specifies, in exhaustive technical detail, how each of these axioms is operationalized in the Humanity Ledger system." },
  { type: "flowchart" }
];
...
export const CHAPTERS: Chapter[] = [
    ...
    sections: [
      {
        ...
        content: [
          ...
          { type: "p", text: "The Humanity Ledger architecture does not evaluate inbound volume statically; rather, it computes I(T) in real time, isolating transactional noise from the mempool to project the directional price vector prior to block finality. This temporal differential  between the intention of the transaction and its irreversible execution  constitutes the system's most valuable analytics window." },
          ...
          { type: "p", text: "Humanity Ledger iteratively reconstructs this state by parsing Mint, Burn, and Swap logs emitted by the EVM. If a ledger's order traverses N ticks, the slippage is modeled by crossing the active liquidity frontier for each tick:" },
          ...
          { type: "p", text: "Sovereign obfuscation frequently entails fragmenting operations to mitigate visible slippage. We define Asymptotic Slippage as the maximum price divergence acceptable by an intelligent router before the operation aborts (revert condition)." },
      ...
      },
      {
        ...
        content: [
          { type: "p", text: "Capital flows are no longer monolithic. Sovereign capital behaves as a compressible fluid flowing across yield gradients operated via cross-chain bridges. Comprehending these flows mandates a mathematical framework that transcends the confines of individual chains." },
          { type: "h4", text: "1.2.1 Net Value Transfer Vectors" },
          { type: "p", text: "Consider a network C_k within a multichain topology. The net exchange in a temporal cluster t is quantified as the vector formed by the sum of inbound and outbound flows across all bridges connected to that network. Humanity Ledger indexes Deposit and Withdrawal events across canonical bridge contracts. The early detection of a scalar asymmetry exceeding the empirical anomaly threshold indicates strategic repositioning, preempting the injection of liquidity into the target L2." },
          { type: "h4", text: "1.2.2 Quantitative Modeling of Finality Latency" },
          { type: "p", text: "Cross-chain mobility introduces temporal hazard: the L1-to-L2 bridge finality window. Ledger arbitrage is constrained by a temporal cost function that integrates stochastic potential devaluation and anticipated volatility during the capital's transit epoch. For Optimistic Rollups, this period is 7 days; for ZK-Rollups, it diminishes to minutes." },
          { type: "p", text: "By capturing the initiation transaction hash on the origin network, Humanity Ledger permits the VIP subscriber to align positions on the destination network well before the oracle finalizes the state. This anticipation represents a temporal analytics advantage absent in standard on-chain data paradigms." },
          { type: "h4", text: "1.2.3 Algorithmic Detection of Covert Accumulation (TWAP/VWAP)" },
          { type: "p", text: "Heavy actors deploy TWAP (Time-Weighted Average Price) or VWAP (Volume-Weighted Average Price) contracts to conceal directional motives. Naive tracking overlooks transfers falling beneath the minimal alert threshold. The mathematical engine of Humanity Ledger unifies these micro-flows by tracking the router's state variable via an exponential decay model:" },
      ...
      },
      {
        ...
        content: [
          { type: "p", text: "The existence of Humanity Ledger introduces the Soros reflexive effect to the protocol: the act of observing the blockchain alters the behavior of the observed entities. A ledger cognizant that its maneuvers spawn real-time alerts capable of moving the market adversely will adopt progressively sophisticated obfuscation strategies. The system must anticipate this adaptive behavior." },
          { type: "h4", text: "1.3.1 Mathematical Modeling of Sovereign Obfuscation" },
          { type: "p", text: "Assume a Ledger W attempting to move V_max from a known entity towards an endpoint E. Recognizing that indexer agents trigger contrary volatility upon sensing the movement, W seeks to maximize the entropy of the transfer graph. The topological routing entropy is defined as:" },
          { type: "math", text: "S(W) = Σᵢ pᵢ log(pᵢ)" },
          { type: "p", text: "Where pᵢ denotes the fraction of the flow routed through intermediate bridge or wallet i. Elevated entropy temporarily mitigates deterministic identification. However, entropy exacerbates both transaction cost and the ledger's operational complexity: a Nash equilibrium exists between the cost of obfuscation and the magnitude of the transposed capital." },
          ...
          { type: "p", text: "To counter the lofty entropy of the obfuscation graph, Humanity Ledger applies dimensionality reduction algorithms tailored for EVM logs. The core hypothesis: multiple EOAs operated by a common sovereign script will share a Behavioral Signature Fingerprint  identical gas tolerance, similar functional chaining order, or constant inter-block latency." },
          { type: "p", text: "Transactions are vectors mapping into a multidimensional space. Through deterministic connectivity algorithms, if the Euclidean distance between the execution signatures of hundreds of anonymous transactions approaches zero, the logical engine collapses them into a singular Syndicated Entity, generating the scalar sum of the aggregate movement and shattering the ledger's obfuscation vector." }
        ]
      }
    ]
  },
  {
    ...
    subtitle: "Engineering the on-chain surveillance stack: deterministic EVM state reconstruction, multi-source RPC architecture, high-efficiency serialization, and sovereign ZKP privacy.",
    sections: [
      {
        id: "2-1",
        title: "2.1 Deterministic Reconstruction of EVM State",
        content: [
          { type: "p", text: "The Ethereum Virtual Machine operates as an uncompromising, deterministic state machine. Given a root state and a transaction block B, the subsequent state is entirely predictable. This determinability anchors the forensic framework of Humanity Ledger: ambiguity does not exist within on-chain data interpretation  there is solely the capacity or incapacity to correctly compute the state transition." },
      ...
      },
      {
        ...
        content: [
          { type: "p", text: "Sovereign access to Humanity Ledger analytics pipelines is furnished via a multi-protocol distribution stack, calibrated to guarantee sub-100ms latency beneath sustained load conditions. This is not marketing hyperbole; it is a relentless engineering requisite driven by the sovereign algorithmic attesting use cases that define the primary demographic." },
          { type: "h4", text: "2.2.1 Multi-RPC Ingestion Topology and Active Failover" },
          { type: "p", text: "A monolithic reliance on a solitary RPC provider constitutes an unacceptable single point of failure. Humanity Ledger topology implements a Source Multiplexer operating over N concurrent RPC nodes. The routing model allocates dynamic weights to every source contingent upon its historical latency and recent error velocity. The heaviest-weighted source is perpetually the primary. Should the error quotient eclipse a critical threshold, that origin is sequestered from the pool pending confirmed recovery across N consecutive successful validations." },
          { type: "h4", text: "2.2.2 Protobuf Serialization: From JSON-RPC to High-Efficiency Binary" },
          { type: "p", text: "Ethereum's JSON-RPC protocol is grossly inefficient for high-frequency paradigms: JSON payloads are plaintext, carrying an exorbitant serialization overhead. For the sovereign conduit, Humanity Ledger transcodes the stream into Google's Protocol Buffers format, yielding an average 3-5x payload contraction and 5-10x serialization acceleration juxtaposed against identical JSON datasets." },
          { type: "code", text: `// Canonical Humanity Ledger Protobuf Schema v4
syntax = "proto3";
package ledger_alert.v4;

message LedgerTransferEvent {
  string tx_hash         = 1;
...
message EntityLabel {
  ...
  float  confidence = 3;
      ...
      },
      {
        ...
        content: [
          { type: "p", text: "Sovereign actors  hedge funds, family offices, heavy-hitting operators  necessitate querying Humanity Ledger's analytics database while cloaking their portfolios of interest, monitoring strategies, and alerting thresholds. This is an architectural metadata vulnerability that conventional API protocols intrinsically fail to alleviate." },
      ...
      },
      {
        ...
        content: [
          ...
          { type: "p", text: "Yields the sovereign tag inextricably bound to a given address, displaying entity nomenclature, strict classification typography (EXCHANGE, DEFI_PROTOCOL, MINER, DAO, EOA, CONTRACT), and an algorithmic confidence percentile bounded within [0.0, 1.0]. The holistic database houses upwards of 50,000 empirically vetted tags, encompassing apex centralised exchanges, system DeFi protocols, expansive mining operators, and governmental actors radiating known on-chain activity." },
          { type: "h4", text: "2.4.4 GET /v4/metrics/sentiment  Derived Sentiment Analytics" },
          { type: "p", text: "Furnishes abstracted market sentiment metrics birthed exclusively from ledger flow distillation. Attributes encompass the 24-hour Buy/Sell pressure index, exchange-inflow vs. exchange-outflow divergence ratios, accrued volumetric density of critical internal transactions, and high-fidelity temporal divergence alerts signaling incongruencies amidst ledger transposition and reactive market pricing vectors." }
        ]
      }
    ]
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          { type: "p", text: "The structural Achilles tendon of myriad on-chain analytics systems revolves strictly around label database integrity. Centralised curation violently introduces vectors of corruption: systemic discrepancies, deliberately malicious artifacts, and clandestine preferential access. The paramount long-term governance protocol of Humanity Ledger proposes the implementation of a Token Curated Registry (TCR): an unfettered decentralised market mechanism wherein participants pledge governance tokens as fiscal collateral to architect, dispute, ratify, or decimate entity labels." },
          ...
          { type: "p", text: "The grand historical ledger documenting ledger transits is perpetually inscribed upon Arweave  an unassailable decentralised permanent-storage topology  utilizing heavily encrypted batch protocols. The cryptographic hashes generated are synchronously anchored inside IPFS across vastly geographically isolated pinning node networks. The irrevocable consequence is a monumental historical archive of ledger interactions structurally immune to censorship, algorithmic modification, or outright obliteration by ANY entity: shielding the data from the Humanity Ledger core team, underlying cloud infrastructures, and absolutely all system regulatory jurisdictions." }
        ]
      },
      {
        id: "3-3",
    ...
    sections: [
      {
        ...
        content: [
          ...
          { type: "p", text: "The stateRoot vector operates as the cryptographic anchor of the overarching Merkle Patricia tree embodying total systemic account realities. Humanity Ledger rigorously audits the stateRoot of every incoming structural block: any fractional mathematical discrepancy segregating the local node instance from the beacon chain's published hash emphatically signals an escalating reorg condition or node mutation, instantly defaulting into automated architectural reconciliation loops." },
      ...
      },
      {
        ...
        content: [
          ...
          { type: "p", text: "Arbitrum's heavily centralised sequencer wields omnipotent capabilities commanding absolute arbitrary batch reordering  directly introducing menacing L2-level MEV vectors canonically coined Sequencer Extractable Value (SEV). Humanity Ledger heuristics incessantly scan for nefarious intra-batch chronological reordering: intercepting towering swaps flawlessly trailed intimately by immediate sandwich execution intra-batch functions as high-fidelity radar exposing aggressive active SEV extraction operations." }
        ]
      },
      {
        id: "4-3",
        ...
        content: [
          ...
          { type: "p", text: "This sprawling 7-day temporal window directly commands and dictates massive ledger capital trajectories: extracting heavyweight assets bridging out from Base back to Ethereum L1 successfully broadcasts immediate L2 initiation signals, yet canonical L1 capital availability remains completely frozen pending outright absolute fulfillment of the exhaustive challenge phase. Highly incentivized Third-party bridge Liquidity Providers heavily shoulder this exact temporal risk strictly fueled by elevated yield premiums." },
          { type: "p", text: "Humanity Ledger surveils hyper-critical withdrawal initiation transmissions flooding the primary L1 OptimismPortal contracts guaranteeing hyper-early detection characterizing massive capital drain abandoning Base heading into L1. Capturing these profound signals literally 7 full daylight cycles prior to tangible L1 capital activation empowers sovereign operators commanding unyielding, insanely lucrative positioning windows fundamentally crushing less-informed general market players." }
        ]
      },
      {
        id: "4-4",
      ...
      },
      {
        ...
        content: [
          ...
          { type: "p", text: "An actively operational Humanity Ledger oracle configuration would have inherently detected the colossal 150m USD Curve withdrawal signaling Maximum Systemic Override alerts radiating essentially 6 daylight hours well before chaotic public-facing contagion completely cratered the market  bestowing an immeasurable window of unparalleled analytics flawlessly permitting elite sovereign operators absolute freedom abandoning terminal, highly-exposed Terra portfolios." },
    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          ...
          { type: "p", text: "Humanity Ledger's 2026 engine implements optimistic parallel execution tracking, predicting state access conflicts before they finalize, enabling microsecond-latency alerts on parallelized networks while maintaining 100% deterministic accuracy." }
        ]
      },
      {
        id: "5-3",
