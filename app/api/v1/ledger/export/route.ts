...
/**
 * @api {get} /api/v1/ledger/export Export historical data to CSV
 */
export async function GET(req: NextRequest) {
  const endpoint = '/api/v1/ledger/export';
  ...
  try {
    ...
    const events = await (prisma as any).globalLedgerEvent.findMany({
      where: {
    ...
    return new NextResponse(csvContent, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="wac_ledger_export_${new Date().toISOString().split('T')[0]}.csv"`,
      },
    });
