...
export async function GET(request: NextRequest) {
    try {
        ...
        // and fetch their latest activity, or just query `users` sorted by totalVolume and map it to a "Humanity Ledger" format.
        ...
        if (!data || !data.users) {
            return NextResponse.json({ status: "empty", ledgers: [] });
        }

        // Transform into a Prediction Ledger feed format
        const ledgers = data.users.map((user: any) => {
        ...
        console.error("Prediction Ledgers API Error:", error);
