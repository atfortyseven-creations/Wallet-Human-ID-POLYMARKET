...
export async function GET() {
    try {
        const cacheKey = 'ledger_leaderboard_500';
        const cached = await redis.get(cacheKey);

        if (cached) {
            const parsed = safeJsonParse(cached, null, 'LEDGER_LEADERBOARD');
            if (parsed) return NextResponse.json({ ledgers: parsed, source: 'cache' });
        }

        // Aggregate volume from LedgerActivity in the last 24h
        const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000);

        // NOTE: LedgerActivity.usdValue is a String field in the Prisma schema 
        ...
        const result = await prisma.ledgerActivity.groupBy({
            by: ['walletAddress'],
            _sum: {
                valueBTC: true,
            },
        ...
        // Fallback for missing Prisma models  ledgerSnapshot and walletAnalytics removed
        ...
        const leaderboard = result.map((r, index: number) => {
            ...
            return {
                ...
                label: snap?.label || `Ledger-${(r.walletAddress ?? '0x000000').slice(2, 6)}`,
                volume24h: volumeUsd,
                volumeBtc,
        ...
        return NextResponse.json({ ledgers: leaderboard, source: 'live' });
    } catch (error) {
        console.error('[API:LedgerLeaderboard] CRITICAL ERROR:', error);
        return NextResponse.json({ error: 'Failed to generate ledger leaderboard' }, { status: 500 });
    }
}
