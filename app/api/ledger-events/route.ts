...
export async function GET(req: Request) {
    try {
        ...
        // do NOT exist in GlobalLedgerEvent schema. Correct fields are:
        // txHash, amountUSD, protocol, timestamp
        const ledgerEvents = await prisma.globalLedgerEvent.findMany({
            take: takeLimit,
            orderBy: {
                timestamp: 'desc'
            },
        ...
        const formattedEvents = ledgerEvents.map(ev => ({
            hash: ev.txHash,
            ...
            action: 'TRANSFER',
            dex: ev.protocol,
            tier: ev.amountUSD >= 1_000_000 ? 'LEDGER'
        ...
        return NextResponse.json(
            { success: false, events: [], error: 'Failed to fetch indexed ledger events.' },
            { status: 500 }
        );
    }
}
