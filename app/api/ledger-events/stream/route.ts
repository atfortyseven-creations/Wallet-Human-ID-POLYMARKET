/**
 * GET /api/ledger-events/stream
 * 
 * Server-Sent Events endpoint that bridges the GetBlock WebSocket ledger 
 * detector (EP2) to browser clients in real time.
 * 
 * Architecture:
 *   GetBlock WSS (EP2)  ledgerEngine singleton  SSE  Browser clients
 * 
...
import { ledgerEngine, LedgerEvent } from '@/lib/blockchain/ledger-ws-engine';
...
export async function GET(req: NextRequest) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(ctrl) {
      ...
      // Subscribe to live ledger events from GetBlock WS
      cleanup = ledgerEngine.subscribe((event: LedgerEvent) => {
        send({ type: 'ledger', ...event });
      });
