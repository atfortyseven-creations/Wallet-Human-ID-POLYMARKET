 ...
 * ------------------------------------
 * High-value ledger data is locked behind a BSV micropayment.
 * Verifies transaction proof of payment and releases full analytics.
 *
 * FIX Bug 21: Removed Math.random() fake ledger address.
 * Now queries the real LedgerActivity database using the intelId (walletAddress)
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const [activities, totalVolume] = await Promise.all([
            ...
            prisma.ledgerActivity.aggregate({
                where: { walletAddress: intelId },
        ...
        if (activities.length === 0) {
            return NextResponse.json({
                ...
                hint: 'This wallet has not been flagged as a ledger by our monitoring system.',
            }, { status: 404 });
        }
