/**
 * GET /api/ledgers/sse
 * Server-Sent Events endpoint  streams ledger transfer events from EP2 WebSocket in real time.
 * Connects GetBlock EP2 (wss://go.getblock.io/95cb42a5aa444537a068031ce279d343) directly to browser.
 *
 * Architecture:
 *   GetBlock WS EP2  ledgerEngine singleton  SSE push  AlertsPanel / LedgerPortfolio
 *
 * Format per event:
...
export async function GET(req: Request) {
    ...
    const stream = new ReadableStream({
        start(controller) {
            ...
            // Subscribe to EP2 ledger events  this starts the WS if not already running
            const unsubscribe = ledgerEngine.subscribe((event) => {
                try {
                    controller.enqueue(
                        encoder.encode(`data: ${JSON.stringify({ type: 'LEDGER_TX', ...event })}\n\n`)
                    );
                } catch {
                    // Client disconnected
                }
