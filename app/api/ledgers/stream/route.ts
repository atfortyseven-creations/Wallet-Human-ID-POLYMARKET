    ...
    if (req.headers.get('accept') === 'text/event-stream') {
        ...
        const stream = new ReadableStream({
            async start(controller) {
                const redis = createRedisClient({ name: 'Ledger-Stream' });
                const pSubscriber = createSubClient('Ledger-Stream-Sub');

                controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'CONNECTED' })}\n\n`));

                // Send latest cache history on connect
                try {
                    const cachedRaw = await redis.get('latest_ledger_alerts');
                    if (cachedRaw) {
                        const alerts = safeJsonParse(cachedRaw, [], 'LEDGER_STREAM_HISTORY');
                        controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'HISTORY', alerts })}\n\n`));
                    }
                } catch (e) {
                    console.error('Failed to fetch ledger cache', e);
                }

                // Explicit types eliminate the three implicit-any errors
                pSubscriber.subscribe('ledger_alerts_stream', (err: Error | null) => {
                    if (err) console.error('Redis sub error:', err);
                });

                pSubscriber.on('message', (channel: string, message: string) => {
                    if (channel === 'ledger_alerts_stream') {
