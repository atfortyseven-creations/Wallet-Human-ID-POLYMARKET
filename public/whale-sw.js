...
const CACHE_NAME = 'whale-chat-v1';
const ASSETS_TO_CACHE = [
  '/',
  '/manifest.json',
  '/icons/icon-192x192.png',
  '/icons/icon-512x512.png',
  '/sounds/whale-sonar.mp3',
];
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    const title = data.hideContent ? 'New Message' : data.title || 'WhaleChat';
    ...
    const options = {
      ...
      tag: data.peerAddress || 'whale-notification', // Groups notifications by sender
    ...
    console.error('[Whale SW] Error parsing push data', e);
  }
});
...
self.addEventListener('sync', (event) => {
  if (event.tag === 'sync-messages') {
    console.log('[Whale SW] Background sync triggered');
    event.waitUntil(processOutbox());
  }
});

async function processOutbox() {
  // Connect to the IndexedDB 'whaleDB' to read the outbox and send via standard fetch/XMTP relay
  // This executes entirely in the background worker when the device regains connectivity.
  return new Promise((resolve) => {
    const request = indexedDB.open('whale-database', 1);
