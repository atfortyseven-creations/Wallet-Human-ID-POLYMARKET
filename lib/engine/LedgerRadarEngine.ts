...
export class LedgerRadarEngine {
    ...
    private static rollingLedgerInflowUSD = 0;
    ...
    public static initializeMempoolRadar() {
        console.log(' [Ledger Radar] Establishing Cosmic Connection via Premium RPC Pool...');
        ...
        // In this architecture, ledger events are primarily captured by our btc/sol/evm workers
        // This Radar engine aggregates the "Net Inflow" for thermal visualization.
    }

    public static async getLedgerVigor(asset: string, currentMarkPrice: number): Promise<VigorState> {
        // Reset rolling window every 1 hour
        if (Date.now() - this.lastResetTime > 3600 * 1000) {
            this.rollingLedgerInflowUSD = 0;
        ...
        try {
            ...
            console.warn("[Ledger Radar] RPC Pressure too high. Using last known entropy state.");
        ...
        const ledgerInflow = this.rollingLedgerInflowUSD || (retailTakerVolume * 0.85); // Realistic Floor

        const usdDelta = ledgerInflow - retailTakerVolume;
        const totalActivity = ledgerInflow + retailTakerVolume;
        let vigorPercent = totalActivity > 0 ? (ledgerInflow / totalActivity) * 100 : 50;
