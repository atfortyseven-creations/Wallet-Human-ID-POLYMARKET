 ...
 */
export const LEDGER_QUEUE_NAME = 'global_crypto_alerts';

export interface LedgerJobData {
  hash: string;
  from: string;
  to: string;
  asset: string;
 ...
 */
export async function addLedgerToQueue(data: LedgerJobData) {
...
export const ledgerQueue = {
    add: async (_name: string, data: LedgerJobData) => addLedgerToQueue(data)
};
