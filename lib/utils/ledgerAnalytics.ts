...
const STORAGE_KEY = 'ledger_analytics';
...
export const ledgerAnalytics = new LedgerAnalytics();

// ─── Convenience Tracking Functions ─────────────────────────────────────────

export const trackMessageSent = (peerAddress?: string) =>
  ledgerAnalytics.track('message_sent', peerAddress ? { peer: peerAddress.slice(0, 10) } : undefined);

export const trackCallStarted = (type: 'audio' | 'video') =>
  ledgerAnalytics.track('call_started', { type });

export const trackCallAnswered = (type: 'audio' | 'video', durationSeconds?: number) =>
  ledgerAnalytics.track('call_answered', { type, ...(durationSeconds ? { durationSeconds } : {}) });

export const trackAttachmentSent = (mimeType: string) =>
  ledgerAnalytics.track('attachment_sent', { mime: mimeType.split('/')[0] });

export const trackContactAdded = () =>
  ledgerAnalytics.track('contact_added');

export const trackStatusPosted = () =>
  ledgerAnalytics.track('status_posted');

export const trackFeatureUsed = (feature: string) =>
  ledgerAnalytics.track('feature_used', { feature });
