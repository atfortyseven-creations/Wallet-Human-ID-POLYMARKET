/**
 * GetBlock WebSocket Ledger Engine  Registry-backed
 * Subscribes to all on-chain ERC-20 Transfer events above $LEDGER_THRESHOLD_USD
 * Feeds the LedgerPortfolio leaderboard and the live Humanity Ledger feed.
...
let backoff = 5000;
const LEDGER_USD_THRESHOLD = parseInt(process.env.LEDGER_THRESHOLD_USD || '50000', 10);
...
/** Singleton WebSocket ledger monitor */
class LedgerWebSocketEngine {
    ...
    connect() {
        ...
        if (activeUrls.length === 0) {
            console.warn('[LedgerEngine] No WSS endpoints available. Stream disabled.');
            return;
        }
        ...
        console.info(`[LedgerEngine] Connecting to WSS: ${url.slice(0, 48)}...`);

        this.ws.on('open', () => {
            console.info('[LedgerEngine] GetBlock WS connected (EP2)');
            this.isAlive = true;
        ...
        this.ws.on('close', () => {
            console.warn('[LedgerEngine] WS disconnected  reconnecting');
            this.stopHeartbeat();
            this.scheduleReconnect();
        });

        this.ws.on('error', (err) => {
            console.error('[LedgerEngine] WS error:', err.message);
        });
    }
    ...
    private handleLog(log: any) {
        ...
        if (usdValue < LEDGER_USD_THRESHOLD) return; // Below threshold
    ...
    private startHeartbeat() {
        ...
        this.heartbeatTimer = setInterval(() => {
            if (this.isAlive === false) {
                console.warn('[LedgerEngine] Ledger Heartbeat missed  terminating');
                this.ws?.terminate();
                return;
            }
            this.isAlive = false;
...
export const ledgerEngine = new LedgerWebSocketEngine();
