...
export default function TestComp() {
  ...
  const pendingId = '';
  const showMobileScanner = false;
  const address = '';
  const emailModalOpen = false;
  const mounted = true;
  const isVerified = false;
  const effectiveIsConnected = false;
...
export default function ConnectPage() {
  ...
  // Z-axis zoom effect on the typography
  ...
  // Blur and opacity for the login assembly
  ...
  return (
    <div ref={containerRef} className="w-full relative bg-white" style={{ height: '200vh' }}>
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center">
        ...
        {/* ── Side-by-side: Login Card (left) + QD Marketing Panel (right) ── */}
        <div className="flex flex-col lg:flex-row items-stretch justify-center gap-5 w-full max-w-[900px]">
                          ...
                          <div className="w-full rounded-xl overflow-hidden border border-[#E0E0E0]">
                            ...
                            <div className="flex items-center justify-center gap-2 py-2.5 px-3 bg-black/[0.025]">
                              {pinCode.split('').map((digit, idx) => (
                                <motion.div key={idx} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.07, duration: 0.3 }} className="w-10 h-11 rounded-lg bg-white border-2 border-black/8 flex items-center justify-center shadow-sm">
          ...
          {/* ── RIGHT: QD Utility Marketing Panel ── */}
          <motion.div
        ...
        <div className="w-full max-w-[900px] flex flex-col gap-4">
          ...
          {/* Platform badges */}
          <div className="bg-white rounded-2xl shadow-lg p-4 flex flex-col gap-3 items-center border border-[#EFEFEF]">
            <div className="text-[8px] font-black uppercase tracking-[0.2em] text-black/35">Coming to all platforms</div>
            <div className="flex items-center gap-3 justify-center flex-wrap">
              {[
                { label: 'iOS',     path: 'M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z' },
                { label: 'Android', path: 'M3.18 23.76c.3.17.65.19.96.07l12.75-7.37-2.79-2.79L3.18 23.76zm-1.13-1.49V1.73c0-.43.23-.82.6-1.03L15.19 12l-13.14 12.3c0-.01 0-.02-.01-.03zm15.53-8.12l2.38 2.38c.41.41.41 1.07 0 1.48L17.72 20l-2.79-2.79 2.65-3.06zM3.18.24c.31-.12.66-.1.96.07L16.9 7.68l-2.79 2.79L3.18.24z' },
                { label: 'Chrome',  path: 'M12 0C8.21 0 4.831 1.757 2.632 4.501l3.953 6.848A5.454 5.454 0 0 1 12 6.545h10.691A12 12 0 0 0 12 0zM1.931 5.47A11.943 11.943 0 0 0 0 12c0 6.012 4.42 10.991 10.189 11.864l3.953-6.847a5.45 5.45 0 0 1-6.865-2.29zm13.342 2.166a5.446 5.446 0 0 1 1.45 7.09l.002.001h-.002l-5.344 9.257c.206.01.413.016.621.016 6.627 0 12-5.373 12-12 0-1.54-.29-3.011-.818-4.364zM12 16.364a4.364 4.364 0 1 1 0-8.728 4.364 4.364 0 0 1 0 8.728Z' },
              ].map(({ label, path }) => (
                <div key={label} className="flex items-center gap-2 px-3 py-2 bg-[#F5F5F5] rounded-xl border border-[#E8E8E8]">
                  <svg viewBox="0 0 24 24" className="w-4 h-4 text-[#0A0A0A]" fill="currentColor"><path d={path} /></svg>
          ...
          <p className="text-white/45 text-[8px] font-mono uppercase tracking-[0.25em] text-center">© 2027 Humanity Ledger · Whale Network</p>
        </div>

      </motion.div>{/* END PHASE 2: Login Assembly */}
