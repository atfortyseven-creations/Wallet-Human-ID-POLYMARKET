...
const bn254_1 = require("@aztec/foundation/curves/bn254");
const aztec_address_1 = require("@aztec/stdlib/aztec-address");
...
async function main() {
    ...
    const wallet = await embedded_1.EmbeddedWallet.create(pxeUrl, { ephemeral: true });
    ...
    const secretKey = bn254_1.Fr.fromHexString(relayerSecretHex.replace(/^0x/i, ''));
    // Use salt = 0 for deterministic address — relayer always gets same Aztec address
    const salt = new bn254_1.Fr(0n);
    ...
    //
    const deployResult = await Token_1.TokenContract
        .deploy(wallet, // Wallet — EmbeddedWallet implements this
