...
export function LegendaryLedgerLeaderboard() {
    const { leaderboard500, ledgerEvents } = useVIPStore();
    ...
    const getLatestTxForWallet = (wallet: string) => {
        return ledgerEvents?.find(e => e.wallet.toLowerCase() === wallet.toLowerCase());
    }
        ...
        <div className="bg-white border border-slate-200 rounded-[2.5rem] overflow-hidden shadow-xl shadow-slate-200/50 group h-full transition-all duration-500 hover:border-cyan-500/30">
            ...
            {rankings.length === 0 && (
                <div className="p-12 text-center space-y-4">
                    ...
                    <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Scanning Blockchain for Ledgers...</p>
                </div>
            )}
