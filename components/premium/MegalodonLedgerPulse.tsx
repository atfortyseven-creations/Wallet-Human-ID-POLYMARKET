...
export function MegalodonLedgerPulse() {
    const [alerts, setAlerts] = useState<UIStoreAlert[]>([]);
    const [isMonitoring, setIsMonitoring] = useState(true);

    // Connect to the REAL on-chain mempool stream  only ledger events ($500k+) arrive here
    useEffect(() => {
        ...
        const fetchPulse = async () => {
            if (!isMounted) return;
            try {
                const res = await fetch('/api/network/ledger/mempool-stream');
                if (res.ok) {
                    const tx = await res.json();
                    if (tx && tx.type === 'ledger') {
    ...
    useEffect(() => {
        fetch('/api/network/ledger/alert-stats')
            .then(r => r.json())
        ...
        <div className="w-full bg-[#0A0A12] border border-white/5 rounded-3xl overflow-hidden shadow-2xl relative">
            ...
            <div className="p-6 border-b border-white/5 bg-white/[0.02] flex justify-between items-center">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-white font-black text-xs uppercase tracking-[0.2em] mb-0.5">Megalodon Ledger Pulse</h3>
            ...
            <div className="p-4 space-y-3 min-h-[400px]">
                            ...
                            <div className="flex justify-between items-start">
                                <div className="flex items-center gap-4">
                                    <div className={`p-2.5 rounded-xl ${
                                        ...
                                        {alert.type === 'LEDGER_TX' ? <Crown className="w-4 h-4" /> : <AlertTriangle className="w-4 h-4" />}
                                    </div>
                                    <div>
                                        <div className="flex items-center gap-2">
                                        ...
                                        <div className="text-[10px] text-white/30 font-mono uppercase tracking-widest mt-1">
                                            {alert.type === 'LEDGER_TX' ? 'Large Transfer Detected' : 'Massive Liquidation Event'}
                                        </div>
                                    </div>
                                </div>
                ...
                {alerts.length === 0 && (
                   <div className="flex flex-col items-center justify-center py-20 opacity-20">
                       ...
                       <p className="text-[10px] font-mono uppercase tracking-[0.2em]">Scanning Ethereum mainnet for ledger events (&gt;$500k)...</p>
                   </div>
                )}
            </div>
