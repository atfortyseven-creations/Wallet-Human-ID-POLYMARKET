...
interface MegaLedgerOverlayProps {
    tx: MegaLedgerTx | null;
    onDismiss: () => void;
}

//  Web Audio API sonic impact engine 
function playMegaLedgerSound(usdValue: number) {
    try {
...
export function MegaLedgerOverlay({ tx, onDismiss }: MegaLedgerOverlayProps) {
    const soundFired = useRef<string | null>(null);

    useEffect(() => {
        if (tx && tx.id !== soundFired.current) {
            soundFired.current = tx.id;
            playMegaLedgerSound(tx.usdValue);
        }
    }, [tx]);
