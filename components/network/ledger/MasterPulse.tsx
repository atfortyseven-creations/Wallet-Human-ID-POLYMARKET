...
interface Props {
  // Props from ledgerData aggregated in the dashboard
  totalLedgerVolume: number;      // BTC moved by ledgers total
...
export function MasterPulse({ totalLedgerVolume, inflowScore, liquidationTotal, activeSatoshiAlerts }: Props) {
  ...
  useEffect(() => {
    ...
    const computeSignal = () => {
      ...
      const ledgerSignal = Math.min(100, totalLedgerVolume / 50); // 5000 BTC = 100
  ...
  }, [totalLedgerVolume, inflowScore, liquidationTotal, activeSatoshiAlerts, rate]);
    ...
    <div className={`w-full rounded-3xl overflow-hidden border transition-all duration-500 ${
      ...
      <div className="grid grid-cols-4 gap-0 border-t border-white/5 mt-4">
        {[
          {
            label: 'Ledger Volume',
            value: `${totalLedgerVolume.toFixed(0)} BTC`,
