...
interface NexusNode {
  id: string;
  label: string;
  type: 'ledger' | 'exchange' | 'unknown' | 'miner';
  btcFlow: number;
  isCenter: boolean;
  x?: number;
  y?: number;
...
export function EntityNexus({ address }: Props) {
  ...
  const fetchData = useCallback(async (addr: string) => {
    ...
    try {
      const res = await fetch(`/api/network/ledger/entity-nexus?address=${addr}`);
      if (res.ok) {
      ...
      <div className="relative">
        {!address ? (
          <div className="flex flex-col items-center justify-center h-72 gap-3">
            ...
            <p className="text-white/20 text-sm">Click on a ledger to view its entity nexus</p>
          </div>
        ) : loading ? (
