...
function useTopLedgerEvents() {
    ...
    const fetch_ = useCallback(async () => {
        try {
            const res = await fetch('/api/top-ledger-events');
...
export function TopLedgerEvents24h() {
    // =========================================================================
    // INJECTED DATA HOOK  Zero-Mock Mandate
    // Ledger events endpoint injected via REGISTRY.Private_INTEL.eventLedger
    // =========================================================================
    const { data: rawData, isLoading, error } = useSystemIntel('eventLedger');
    const events: LedgerEvent[] = rawData?.data ?? [];
            ...
            <div className="flex-1 overflow-auto divide-y divide-[#F0F0F0]">
                {isLoading ? (
                    ...
                    <div className="p-16 text-center text-[#888888]">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest">No major ledger events in the last 24h</p>
                    </div>
                ) : (
