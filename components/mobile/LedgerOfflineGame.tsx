...
const CW = 360, CH = 260;
const GROUND = CH - 55;
const LEDGER_X = 65, LEDGER_W = 46, LEDGER_H = 34;
...
function mkState(): GS {
  return { wy: GROUND - LEDGER_H, wvy: 0, onGround: true, obstacles: [],
...
function drawLedger(ctx: CanvasRenderingContext2D, s: GS, img: HTMLImageElement|null) {
  ctx.save();
  const cx = LEDGER_X + LEDGER_W/2, cy = s.wy + LEDGER_H/2;
  const rot = Math.max(-.28, Math.min(.28, s.wvy * .018));
  ctx.translate(cx, cy); ctx.rotate(rot);
  if (s.dead) { ctx.globalAlpha=.6; ctx.rotate(Math.PI*.18); }
  if (img) ctx.drawImage(img, -LEDGER_W/2, -LEDGER_H/2, LEDGER_W, LEDGER_H);
  else {
    ctx.fillStyle='#1A1A2E';
    ctx.beginPath(); ctx.ellipse(0,0,LEDGER_W/2,LEDGER_H/2,0,0,Math.PI*2); ctx.fill();
  }
  ctx.restore();
}
...
//  COMPONENT 
export function LedgerOfflineGame({ visible, onBack }: { visible: boolean; onBack?: () => void }) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (s.started && !s.dead) {
        ...
        if (s.wy >= GROUND - LEDGER_H) { s.wy = GROUND - LEDGER_H; s.wvy = 0; s.onGround = true; }
        ...
        const wx1=LEDGER_X+10, wx2=LEDGER_X+LEDGER_W-10, wy1=s.wy+8, wy2=s.wy+LEDGER_H-6;
        for (const o of s.obstacles) {
          if (wx2>o.x+5 && wx1<o.x+o.w-5 && wy2>o.y+5 && wy1<o.y+o.h) {
            s.dead = true; setUiDead(true);
            const fs = Math.floor(s.score);
            if (fs > hsRef.current) { hsRef.current=fs; setUiHi(fs); try{localStorage.setItem('ledger_hs',String(fs));}catch{} }
            break;
          }
        }
      ...
      drawLedger(ctx, s, imgRef.current);
