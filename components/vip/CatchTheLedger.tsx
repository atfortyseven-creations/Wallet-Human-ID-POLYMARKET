...
export function CatchTheLedger() {
    ...
    const handleSearch = async (e: React.FormEvent) => {
        ...
        setSearchError(null);
        try {
            const res = await fetch(`/api/network/ledger/search?address=${encodeURIComponent(query.trim())}`);
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-4">
                    <h1 className="text-5xl sm:text-8xl font-black uppercase tracking-tighter leading-none text-[var(--aztec-ink)]">
                        Catch the <span className="text-[var(--aztec-ink)]/20">Ledger</span>
                    </h1>
                </div>
            </div>
